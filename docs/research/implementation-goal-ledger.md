...
| Coût par résultat accepté | Partiel vérifié web/desktop + mobile + procédure dogfood | Cumul des reprises figé ; **`human_effort_seconds`** web/desktop + mobile ; comparaison A/B miss→corriger vs hit→Accept (effort 4s vs 1s ; delay mural distinct) ([human-effort-procedure-2026-09-07.md](human-effort-procedure-2026-09-07.md)) ; reste n plus large / équipes externes |
| Alertes et plafonds | **Plafond Multica** (pas USD fournisseur) | Plafonds durs Multica + alertes ~80 % documentés. **Fermé pour USD fournisseur** : hors contrôle Multica. |
| Mémoire par agent | **Plafond dogfood** (publish humain) | DEV-3/4 + DEV-5 never-eval. **Fermé pour « apprentissage autonome »** : hors produit (publish/promote humain requis) — [project-memory-cross-agent-2026-09-07.md](project-memory-cross-agent-2026-09-07.md). |
| Mémoire partagée de projet | Partiel vérifié (promo + reuse + cross-agent + retry↓) | DEV-6 + retry↓ cumul **n=8+8** (vagues keel/mast/rift) −100 % relatif synthétique ([memory-retry-reduction-2026-09-07.md](memory-retry-reduction-2026-09-07.md)). Apprentissage autonome **hors produit**. Reste familles métier / équipes externes. |
| Compétence candidate → replay → cas réservés → promotion réversible | **Plafond dogfood** (estimation catalogue) | Hors ligne + UI + pilotes Codex/Claude ; revue humaine 18 s ; `report_hash` ; `catalog_estimate` honnête. **Fermé pour cette campagne** : pas de facture fournisseur dans le produit Multica — hors scope. |
