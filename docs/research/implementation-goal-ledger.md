...
| Mémoire partagée de projet | Partiel vérifié (promo + reuse + cross-agent + retry↓) | Publication/scoping/expiration/historique, promotion `source_review`, usage API/UI ; DEV-6 cross-agent ; retry↓ cumul **n=6+6** (`keel-4` 2+2 + `mast-9` 4+4) : baseline 6/6 reprises → treatment 0/6 (−100 % relatif, synthétique) ([memory-retry-reduction-2026-09-07.md](memory-retry-reduction-2026-09-07.md)) ; apprentissage autonome hors produit ; reste familles métier réelles / équipes externes |
