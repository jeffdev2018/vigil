...
This means another in-flight run is using the same local directory. Multica waits for the directory lock to be released so two agents never modify the same files at once. The issue **execution log** shows the hold cause (directory / holding task when known) and the next action on the status hover.
...
### Retry from the execution log

A manual **Retry** on a failed or cancelled row starts a new run for that same agent. It reuses the prior working directory when still available, and resumes the prior agent session only when that failure did not poison the conversation; otherwise it starts a fresh session. Comments, pull requests, and other side effects the earlier run already produced are **not** undone and may be repeated — Multica does not roll those back.
