...
| Lier automatiquement tâches et PR | Associe une PR à la tâche dont l'identifiant figure dans son nom de branche ou son titre. |
...
Le fait qu'une PR fusionnée termine sa tâche est un réglage distinct, propre à l'espace de travail : **Paramètres → États des tâches → Automatisation → Terminer les tâches quand leurs PR sont fusionnées**. Il s'applique à tous les hébergeurs de code, y compris les [fournisseurs Git auto-hébergés](/vcs-integration).
...
Le corps de la PR n'est pas lu, et les mots-clés n'ont aucun sens particulier : `Closes MUL-123` ou `Related to MUL-123` dans la description n'associe rien. Les messages de commit et les commentaires de PR ne déclenchent pas non plus l'association.

Vous pouvez aussi associer une PR à la main : dans le bloc **Pull requests** de la tâche, cliquez sur **+** et collez l'URL de la PR. Pour retirer une PR d'une tâche, ouvrez le menu **⋯** sur sa ligne et choisissez **Retirer de la tâche** ; Multica ne la réassociera pas automatiquement depuis son titre ou sa branche.
...
Sous la liste, une ligne indique ce qui se passera ensuite — par exemple *Se termine quand #19 est fusionnée*, ou quelle PR a été fermée sans fusion.
...
Avec **Terminer les tâches quand leurs PR sont fusionnées** activé (réglage par défaut), une tâche passe à `Terminé` quand toutes les PR qui lui sont associées sont fusionnées :

1. la tâche a au moins une PR associée, et toutes les PR associées sont `Fusionnée` — une PR `Ouverte` ou en `Brouillon` la laisse en attente, tout comme une PR fermée sans fusion tant qu'elle n'a pas été retirée de la tâche ;
2. la tâche n'est ni `done` ni `cancelled`, et la fin automatique n'a pas été désactivée pour elle (**Pull requests → ⋯ → Ne pas terminer cette tâche automatiquement**), pour le travail qui a besoin d'une relecture ou d'une mise en production après la fusion.

La vérification s'exécute quand un événement de PR touche la tâche : une PR associée est fusionnée, une PR est associée, ou une association est retirée. Rouvrir une tâche ne la ramène pas à `Terminé` — c'est la prochaine PR associée qui fusionne qui le fait. Activer le réglage ne termine pas les tâches dont les PR ont fusionné avant.

Le changement de statut est inscrit dans la chronologie de la tâche comme une action système nommant les PR, et les membres abonnés à la tâche sont notifiés.
...
- **PR non associée** : vérifiez que le dépôt fait partie du périmètre autorisé de l'App, que l'association automatique est activée, que l'identifiant appartient à l'espace de travail courant, et que personne n'a retiré la PR de la tâche.
- **Identifiant présent seulement dans la description** : placez-le dans le nom de branche ou le titre de la PR, ou associez la PR à la main.
...
- **Tâche non terminée après la fusion de la PR** : lisez la ligne sous la liste **Pull requests** de la tâche. Elle nomme toute PR encore ouverte ou fermée sans fusion, ou indique que la fin automatique est désactivée pour la tâche ou pour l'espace de travail.
