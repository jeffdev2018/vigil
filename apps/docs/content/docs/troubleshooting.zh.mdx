...
这种等待只存在于目录的 `in_place`（“原地”）模式。如果这个目录是 git 仓库，把资源切到 `worktree`（“并行”）就没有队列了：每次运行拿到自己的 worktree，并以分支形式交回结果，谁都不用等谁。见[项目资源](/project-resources#task-如何共享这个目录)。
...
当 `dispatched` 或 `running` 的运行超过存活阈值（默认 90 秒）没有守护进程报告的消息或进度时，执行日志会把该运行标为 **无响应** 徽章。这个徽章只是根据运行的最近活动实时计算出来的，从不持久化：运行状态保持不变，不会因为沉默而判定失败，工具发来下一条消息后徽章就会消失。

一次较长的编译或较慢的工具调用也可能在健康的运行上触发这个徽章。如果徽章持续了几分钟，检查该运行的守护进程日志；如果工具确实卡住了，就从执行日志里停止这次运行。自托管服务器可以用 `MULTICA_RUN_UNRESPONSIVE_AFTER` 按秒调高或调低这个阈值；客户端在启动时从 `/api/config` 读取该值。
