...
这表示另一条在途运行正在使用同一个本地目录。Multica 会等待目录锁释放，避免两个智能体同时修改同一份文件。任务的**执行日志**会显示等待原因（目录 / 占用中的运行，若已知），并把下一步操作放在状态悬停提示里。
...
对失败或已取消的行点**重试**，会为同一智能体启动一次新运行。在可用时会复用先前的工作目录；仅当上次失败未污染会话时才会恢复先前会话，否则从新会话开始。先前运行已经产生的评论、PR 和其他外部副作用**不会**被撤销，并可能再次发生——Multica 不会回滚这些结果。
