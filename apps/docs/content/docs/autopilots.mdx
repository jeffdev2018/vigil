...
When the workspace automation-run ceiling is enforcing, Multica refuses to admit a new autopilot run for the period (completed and reserved runs both count). That is a Multica admission refusal, not a provider-quota error on an already-running CLI. See [Usage limits and ceilings](/workspaces#usage-limits-and-ceilings).
