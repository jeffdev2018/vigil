...
| `MULTICA_DECISIONS_API_KEY` | 空 | 型付き判定エンドポイントの API キー |
| `MULTICA_DECISIONS_BASE_URL` | 空 | 判定エンドポイントのパスまで含んだ完全な URL。判定モデルは chat/completions を受け付けないため、`MULTICA_LLM_BASE_URL` では到達できません |
| `MULTICA_DECISIONS_MODEL` | 空 | 使用する判定モデル |
...
型付き判定は、上記のアシストレイヤーとは別のエンドポイントです。判定モデルは閉じた質問 — 名前付き選択肢からの 1 つ、順序付きレベルによる評点、確率 — にのみ答え、呼び出し側が与えたスキーマの外を返すことはできません。3 つの変数すべてが空なのが既定で、その場合は無効となり、各呼び出し元はこのエンドポイントが存在しなかったときの動作をそのまま保ちます。現在の利用箇所は 1 つで、ゴールループの判定です。ゴール、イシューのタイトルと説明の抜粋、終了した実行の終了ステータスを送り、ゴールが達成されたかを問います。
