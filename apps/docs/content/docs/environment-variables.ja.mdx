...
Redis は、リアルタイムイベント（開いているブラウザタブへ push されるタスクとチャットの更新）のファンアウトも支えます。これらは `REDIS_URL` とは独立しており、プロダクションではリクエスト経路のキャッシュから切り離しておく価値があります。
...
- **タスクのスコープ設定アシスタント**（タスク作成ダイアログの「数文からドラフトを生成」） — メンバーが入力したテキスト（最大 8000 文字）と、対象プロジェクトのタイトルおよび説明を送信します。
- **[ネイティブランタイム](/native-runtime)** — デーモンなしで動くエージェントのツール呼び出しループが、タスクとコメントの内容を同じクライアントへ送信します。ここでは `MULTICA_LLM_ROUTING_MODEL` は使われません。ネイティブランタイムの呼び出しは `MULTICA_LLM_DEFAULT_MODEL` を使います。
