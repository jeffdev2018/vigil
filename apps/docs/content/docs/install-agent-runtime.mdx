...
After installing, complete the tool's sign-in or model provider setup. For Claude Code and Codex you can do that from Multica: open the **Runtimes** page, find the runtime under its machine, and start the sign-in there. The daemon runs the CLI's device-code login on that computer and the dialog shows the verification URL and one-time code; the same place signs the CLI out again. Every other tool has to be launched once in a terminal on the computer itself. If a tool cannot complete requests in the terminal, the daemon's invocations fail the same way.
