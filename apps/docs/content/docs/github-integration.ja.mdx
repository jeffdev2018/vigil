...
| PR の自動紐づけ | PR のブランチ名、タイトル、本文から課題番号を検出します。 |
...
課題番号を PR 本文だけに書く場合は、GitHub の closing keyword を使ってください。
...
本文に `Related to MUL-123` のような通常の参照を書いただけでは、その課題の作業 PR として表示されません。Commit message と PR コメントも紐づけの対象外です。
...
2. その課題に、`Open` または `Draft` の作業 PR がほかに残っていない。本文中の通常の参照は作業 PR に含みません。
