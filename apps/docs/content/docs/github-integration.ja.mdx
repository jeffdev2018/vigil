...
| PR の自動紐づけ | PR のブランチ名とタイトルから課題番号を検出します。本文では closing keyword の直後にある番号だけを検出します。 |
...
課題番号を PR 本文だけに書く場合は、GitHub の closing keyword を必ず使ってください。
...
本文に `Related to MUL-123` のような通常の参照を書いただけでは、その課題へまったく紐づきません。Commit message と PR コメントも紐づけの対象外です。
...
PR がマージされても、必ずしも課題が完了するわけではありません。Multica が課題を `Done` に変更するのは、次の条件をすべて満たす場合だけです。

1. 紐づいたマージ済み PR のうち少なくとも 1 つで、`Closes MUL-123` のように closing keyword の直後に課題番号を指定している。`Closes login MUL-123` のように間に別の単語がある形式は無効です。
2. その課題に、`Open` または `Draft` の紐づいた PR がほかに残っていない。
3. 課題の現在の状態が `done` または `cancelled` ではない。

そのため、ブランチ名やタイトルに `MUL-123` を含めるだけで紐づけは作成されますが、それだけでは課題を完了にしません。PR をマージせずにクローズした場合も、課題は完了しません。

状態の変更はシステム操作としてタイムラインに記録され、その課題を購読しているメンバーへ通知されます。
...
- **本文に番号を書いても紐づかない**: `Closes MUL-123` を使うか、番号をブランチ名または PR タイトルに入れます。
...
- **PR のマージ後も課題が完了しない**: PR で closing keyword を使っていることと、`Open` または `Draft` の関連 PR がほかに残っていないことを確認します。
