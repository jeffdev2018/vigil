---
title: GitHub 連携
description: pull request を Multica の課題に紐づけ、開発の進捗を課題から確認します。
---

import { Callout } from "fumadocs-ui/components/callout";

GitHub を接続すると、Multica は課題番号を使って pull request を自動的に紐づけます。課題の詳細画面から、PR の状態、変更規模、CI の結果、マージ競合を直接確認できます。
...
<Callout type="info">
**GitHub 接続**は、Multica がどのリポジトリから PR イベントを受け取るかを決めます。**コードリポジトリ**設定は、エージェントが作業を実行するときに選べるリポジトリを決めます。用途が異なるため、それぞれ設定する必要があります。
</Callout>
...
最も簡単な方法は、課題番号をブランチ名または PR タイトルへ入れることです。課題が `MUL-123` の場合:
...
Multica は大文字と小文字を区別せず、現在のワークスペースの課題接頭辞だけを照合します。1 つの PR を複数の課題に紐づけることもできます。

Multica は PR 本文を読み取らず、キーワードにも特別な意味はありません。本文に `Closes MUL-123` や `Related to MUL-123` と書いても紐づきません。Commit message と PR コメントも紐づけの対象外です。

手動で紐づけることもできます。課題の **Pull requests** セクションで **+** をクリックし、PR の URL を貼り付けます。PR を課題から外すには、その行の **⋯** メニューから **課題から外す** を選びます。以後、Multica がタイトルやブランチから再び紐づけることはありません。

## 課題から PR を確認する

紐づけが完了すると、PR が課題詳細の **Pull requests** セクションに表示されます。各項目には次の情報が含まれます。
...
リストの下には、次に何が起きるかを 1 行で表示します。たとえば「#19 がマージされると自動完了」や、マージされずにクローズされた PR です。

項目をクリックすると GitHub の PR が開きます。**PR サイドバー**を無効にしてもこのセクションが非表示になるだけで、接続は解除されません。

## PR のマージで Done になる条件

**PR のマージで課題を自動完了**（**設定 → 課題ステータス → 自動化**、既定でオン）が有効な場合、課題に紐づくすべての PR がマージされると、課題は `Done` になります。

1. 課題に紐づく PR が 1 つ以上あり、すべてが `Merged` である。`Open` や `Draft` の PR が残っていると課題は待機を続けます。マージされずにクローズされた PR も、課題から外すまで同様です。
2. 課題が `done` または `cancelled` ではなく、その課題だけ自動完了をオフにしていない（**Pull requests → ⋯ → この課題は自動完了しない**）。マージ後にレビューやリリースが必要な作業向けです。

判定は PR イベントが課題に関わったときだけ行います。紐づいた PR のマージ、紐づけの追加、紐づけの解除です。課題を再度開いても `Done` には戻らず、次に紐づいた PR がマージされたときに完了します。設定をオンにしても、以前にマージされた課題をさかのぼって完了にはしません。

状態の変更は関連する PR を示したシステム操作としてタイムラインに記録され、その課題を購読しているメンバーへ通知されます。この設定は[セルフホスト Git ホスティング](/vcs-integration)を含むすべてのコードホストで共通です。

## 複数のワークスペース

同じ GitHub App installation を複数の Multica ワークスペースへ接続できます。GitHub イベントは各ワークスペースにそれぞれ送られ、各ワークスペースの課題接頭辞に従って照合されます。

たとえば、1 つの PR が `MUL-1` と `ENG-2` の両方を参照している場合、異なる接頭辞を持つ 2 つのワークスペースでそれぞれ紐づけられます。ワークスペース間で相手の課題が見えることはありません。

同じ番号が接続中の複数のワークスペースで課題に一致する場合（接頭辞が同じときに起こり得ます）、Multica はどのワークスペースでも自動では紐づけません。関係のない課題がマージで完了しないようにするためです。正しいワークスペースで手動で紐づけてください。
...
秘密鍵は GitHub App の **Private keys → Generate a private key** から生成します。設定しない場合も連携は安全に縮退します。PR のミラー、課題の自動紐づけ、マージ時の Done への変更は動作しますが、PR カードに CI やマージ状態は表示されません。
...
- **PR が紐づかない**: リポジトリが App の許可範囲にあること、PR の自動紐づけが有効であること、番号が現在のワークスペースのものであること、誰かが課題から外していないかを確認します。
- **番号を本文にだけ書いている**: 番号をブランチ名または PR タイトルに入れるか、手動で紐づけます。
...
- **PR のマージ後も課題が完了しない**: 課題の **Pull requests** リストの下にある説明を確認します。まだマージされていない PR やマージされずにクローズされた PR、または課題やワークスペースで自動完了がオフになっていることが表示されます。

## 次のステップ

- [課題](/issues) — 状態遷移と PR マージによる Done への変更
