...
| `MULTICA_DECISIONS_API_KEY` | empty | API key for the typed-decision endpoint |
| `MULTICA_DECISIONS_BASE_URL` | empty | Full URL of the decision endpoint, path included. A decision model refuses the chat/completions surface, so it cannot be reached through `MULTICA_LLM_BASE_URL` |
| `MULTICA_DECISIONS_MODEL` | empty | Decision model to ask |
...
Typed decisions are a separate endpoint from the assist layer above. A decision model answers a closed question — one option out of a named set, a rating against ordered levels, or a probability — and cannot reply outside the schema the caller gave it. All three variables empty is the default and leaves it disabled, in which case every caller keeps the behaviour it had before the endpoint existed. One caller uses it today: the goal-loop judge, which sends the goal, the issue title and description excerpt, and the finishing run's closing status, and asks whether the goal is met.
