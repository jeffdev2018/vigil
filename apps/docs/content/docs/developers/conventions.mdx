...
- For UUID parsing in handlers, follow the rule in the root `AGENTS.md` — `parseUUIDOrBadRequest` for boundary input, `parseUUID` (panicking) for trusted round-trips, never `util.ParseUUID` directly without checking the error.
...
| Entity | en | zh-Hans | ja | ko | fr |
| --- | --- | --- | --- | --- | --- |
| the tracked unit of work (`issue`) | Issue | **任务** | **課題** | **이슈** | **Tâche** |
| one agent execution record (`task` in API / DB) | **Run** | **运行** | **実行** | **실행** | **Exécution** |

Both concepts are user-visible and they are **not** the same thing — an issue can carry many runs. Never let one locale spell both with the same word. `Task` is no longer a user-facing product noun for an agent run; it remains only as the established internal identifier.

What does not move:

- **API / DB fields** stay `issue` / `task` / `skill` everywhere: `issue_status`, `task_id`, `skill_uuid`. Developer prose calls the product object a Run and may clarify it as “Run (`task_id` in the API)” when the identifier matters.
...
**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese or Japanese outside of dev jargon. Chinese uses the everyday task word (任务), Japanese the word for a matter to be handled (課題), and Korean the loanword 이슈, which Korean product UI already uses. This is what every locale file and every localized doc page actually says; an earlier row of this table (タスク / 태스크) had been reverted in the strings the same day it landed and never in the glossary. `skill` is a Multica-specific concept where no local word carries the meaning.
...
- **`autopilot` → "自动化"**: in Chinese, "autopilot" associates with Tesla's "自动驾驶" and doesn't match what the feature does (start agent runs on a schedule). Notion and Feishu both use "自动化"; that's the industry consensus.
...
- Issue status category: `unstarted` / `started` / `done` / `closed`
...
Descriptions are optional and omitted by default. Before adding one, ask: **What would the user misunderstand or do incorrectly without this sentence?** If the title, label, control, value, or button already answers it, omit the sentence.

- State a fact once, next to the relevant control. Do not repeat it across page, section, row, empty state, and dialog descriptions.
- Prefer a precise label or example to a paragraph explaining an obvious action. Avoid generic instructions such as "Create one to get started", "Click below", or "You can change this later" unless they resolve a real ambiguity.
- Keep permissions, cost, destructive consequences, execution prerequisites, input constraints, and error recovery visible when relevant. Do not hide them in hover-only help.
- Show introductory guidance only where it is needed. Put advanced usage and diagnostics in explicit, accessible help rather than the default page body.
- Preserve input labels, accessible names, and necessary description associations. Do not move redundant visible prose wholesale into screen-reader-only text.
- Review English and translated copy together, including mobile's independent strings. In PR review, check the added information, same-screen repetition, display conditions, and preserved constraints. Length is a review signal, not an automatic deletion rule.

Shared settings components already accept optional descriptions. Examples and new call sites should omit them unless justified; do not add required description props or a separate copy policy.
