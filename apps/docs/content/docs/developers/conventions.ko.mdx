...
- **일상어** — 사용자가 소리 내어 말하는 단어입니다. 데이터베이스 엔티티인지와 무관하게 **완전히 번역**합니다: `issue` → 이슈, `workspace` → 워크스페이스, `project` → 프로젝트.
...
### `issue`는 제품의 "이슈" — 번역합니다

`issue`는 사용자가 등록하고 에이전트가 처리하는 대상의 영어 제품 이름입니다. 영어가 아닌 모든 로케일에서는 그 언어에서 **일상적으로 이 대상을 가리키는 단어**가 됩니다:

| 엔티티 | en | zh-Hans | ja | ko | fr |
| --- | --- | --- | --- | --- | --- |
| 등록된 작업 단위(`issue`) | Issue | **任务** | **課題** | **이슈** | **Tâche** |
| 에이전트의 한 번 실행 기록(API / DB에서는 `task`) | **Run** | **运行** | **実行** | **실행** | **Exécution** |

두 명사 모두 사용자에게 보이며, **같은 것이 아닙니다** — 하나의 이슈에 여러 실행이 연결될 수 있습니다. UI와 문서에서는 실행 기록을 `Task`나 "작업"으로 부르지 않고 **Run / 실행**을 사용합니다. `task`는 기존 API / DB 식별자로만 유지합니다.
...
**왜 `issue`는 번역하고 `skill`은 번역하지 않는가**: 사용자는 매일 issue를 등록하고 읽습니다. 그리고 "issue"라는 단어는 중국어·일본어·한국어에서 개발자 은어를 빼면 아무 의미가 없습니다. 중국어는 일상적인 태스크 단어(任务)를, 일본어는 처리해야 할 사안을 뜻하는 단어(課題)를, 한국어는 이미 한국 제품 UI에서 쓰이는 외래어 이슈를 씁니다. 이는 모든 로케일 파일과 모든 현지화 문서 페이지가 실제로 쓰는 표현입니다; 이 표의 이전 행(タスク / 태스크)은 반영된 당일 문자열에서 되돌려졌고 용어집에는 반영된 적이 없습니다. `skill`은 그 의미를 담을 현지어가 없는 Multica 고유 개념입니다.
...
- 이슈 상태 카테고리: `unstarted` / `started` / `done` / `closed`
