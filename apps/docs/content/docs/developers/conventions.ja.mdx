...
すべての課題には `MUL-123` のような人が読めるキーがあります。ワークスペースの `issue_prefix`（大文字と数字、通常 3 文字、最大 10 文字）+ 連番です。ワークスペースの admin は Settings → General で接頭辞を変更できますが、変更すると既存のすべての課題が番号を振り直されるため、古い接頭辞が埋め込まれた外部参照（PR タイトル、ブランチ名、ドキュメントやチャット内のリンク）は解決されなくなります。
...
- **日常語** — ユーザーが口に出して言う言葉。データベースのエンティティかどうかに関わらず**完全に翻訳**します: `issue` → 課題、`workspace` → ワークスペース、`project` → プロジェクト。
...
`issue` は、ユーザーが登録しエージェントが取り組む対象の英語での製品名です。英語以外のロケールでは、その言語で**日常的にこの対象を指す語**になります。

| エンティティ | en | zh-Hans | ja | ko | fr |
| --- | --- | --- | --- | --- | --- |
| 登録された作業の単位（`issue`） | Issue | **任务** | **課題** | **이슈** | **Tâche** |
| エージェントの 1 回の実行記録（API / DB では `task`） | **Run** | **运行** | **実行** | **실행** | **Exécution** |

どちらの名詞もユーザーの目に触れ、しかも**同じものではありません** — 1 つの課題に複数の実行が紐づくことがあります。UI とドキュメントでは、実行記録を `Task` や「作業」と呼ばず、**Run / 実行**を使います。`task` は既存の API / DB 識別子としてのみ残します。
...
**なぜ `issue` は翻訳し、`skill` は翻訳しないのか**: ユーザーは毎日 issue を登録し、読んでいます。そして "issue" という語は、中国語・日本語では開発者の隠語以外に意味を持ちません。中国語は日常語の「任务」を、日本語は取り組むべき事柄を指す語「課題」を使い、韓国語はすでに韓国語の製品 UI で使われているローンワード「이슈」を使います。これは各ロケールのファイルとローカライズされたすべてのドキュメントページが実際に使っている語であり、この表の以前の行（タスク / 태스크）は、文字列に反映された当日のうちに差し戻され、用語集に載ることは一度もありませんでした。`skill` は、その意味を担える現地語が存在しない Multica 固有の概念です。
...
- 課題の状態: `backlog` / `todo` / `in_progress` / `in_review` / `done` / `blocked` / `cancelled`
- 課題の状態カテゴリ: `unstarted` / `started` / `done` / `closed`
