...
本页聚焦实现，因此正文中的 **task** 指产品 **Run**（运行）背后的内部调度、API 和数据库实体。界面与产品文档统一称为 Run；`TaskService`、`task_id` 等标识为兼容性保持不变。
...
跨应用智能体协作（开放 Agent2Agent 协议）是规划方向，不是当前行为。见 [未来演进 — Agent2Agent 互通](/developers/future-agent2agent)。

## 接下来

- [参与开发](/developers/contributing) — 本地环境、worktree 和测试位置。
- [开发规范](/developers/conventions) — 命名、术语和中文文案的仓库契约。
- [未来演进 — Agent2Agent 互通](/developers/future-agent2agent) — 跨应用智能体（backlog）。
