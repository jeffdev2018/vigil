...
The ordinary list is newest first, which can bury something worth acting on
now under things that can wait until tomorrow. **Needs your attention**
re-orders the same notifications by risk instead of date: nothing extra is
stored, the order is computed each time you open it.

A row at the top of the ordinary inbox list — with a live count — appears
whenever at least one item needs it; opening it switches to the risk-ordered
view. Higher risk comes from a more severe item (`action_required` outranks
`attention`), a pending [Decision Card](/decisions) (higher still at `high`
urgency), a decision that already escalated past its deadline (the top of the
list, regardless of its urgency), and, to a smaller degree, how long an item
has waited — so nothing sits unseen near the bottom indefinitely.

The same projection is available directly: `GET /api/inbox/attention`.
...
Selecting a notification marks it as read. Archive it when you're done; archiving doesn't modify or delete the corresponding issue. Once an issue reaches a status that hands it back to a person — `in_review`, a custom status in the `started` category, or any status in the `done` or `closed` category — its run-failure notifications archive automatically. `blocked` is the exception: it hands the issue over, but the work is still stuck, so past failures stay worth reading.
...
Open **Archived** at the bottom of the inbox to browse older notifications. More records load as you scroll. Filters apply to the entire archive, including records that have not loaded yet.
...
- [Decisions](/decisions) — Decision Cards and requirement interviews, and the Decisions view that answers them from the inbox.
