...
multica issue comment update <comment-id> --expected-revision 2 --content "Check the regression tests and release notes"
...
View the messages of a single run, or stop one:
...
A run publishes the checklist it is working through with `run-plan set`. Each `--item` is `<text>:<status>`, where the LAST colon separates the status (`pending`, `in_progress` or `done`) — so the text may contain colons of its own — and at most one item may be `in_progress`. Publishing replaces the whole plan; only the run itself can publish its own, while it is still running:
...
# ...or as JSON on stdin, with no --item
echo '{"items":[{"text":"Fix the parser","status":"in_progress"}]}' \
  | multica issue run-plan set "$MULTICA_TASK_ID"
```
...
Deleting a runtime that still has active agents bound to it is refused by default. `multica runtime delete <id> --cascade` unbinds those agents, preserves their configuration and history, and cancels their active runs.
...
| `pack` | Install, upgrade, remove and export function packs |
...
| --- | --- | --- | --- |
| `issue` | `list` | List issues | `--status`, `--priority`, `--assignee`, `--project`, `--metadata` (repeatable), `--property` (repeatable, `"Name=Value"`; `__none__` matches unset), `--limit`, `--offset`, `--sort` (incl. `property:<name-or-id>`), `--full-id`, `--fields` (JSON only; comma-separated issue fields), `--resolve-properties` (JSON only; rows with property, option and member names beside the stored ids) |
| | `get <id>` | Show a single issue | `--resolve-properties` (JSON only; rows with property, option and member names beside the stored ids) |
| | `create` | Create an issue | `--title` (required), `--description` / `--description-stdin` / `--description-file`, `--status`, `--priority`, `--assignee`, `--delegate` (the assignee's partner; starts no run), `--parent`, `--stage`, `--project`, `--start-date`, `--due-date`, `--attachment` (repeatable) |
...
| | `ask-agent <issue-id>` | Send an agent-to-agent message on an issue | `--to`, `--intent` (`question`, `review`, `handoff`), `--body` / `--body-stdin` / `--body-file` |
...
| | `run-plan set <task-id>` | Publish (and replace) the run's own plan | `--item "<text>:<status>"` (repeatable; JSON on stdin otherwise), `--issue` |
| | `usage <issue-id>` | View aggregated token usage | |
| | `spend-token request <run-id>` | Ask for a spend token, or collect one for an already-approved spend gate | `--amount`, `--purpose`, `--gate`, `--wait`, `--issue` |
| | `spend-token verify <run-id>` | Spend a token: declare the charge it is covering | `--token`, `--amount`, `--issue` |
| | `budget-status <run-id>` | Show a run's usage against its limits, and the limits it has hit | `--issue` |
| | `rerun <id>` | Re-enqueue a run for the current assignee | |
| | `cancel-task <task-id>` | Cancel a running or queued run | `--issue` |
...
| | `export <id>` / `import <DAEMON.md>` | Export an autopilot as DAEMON.md, or import one | `import`: `--strategy` (`fail`, `overwrite`, `rename`), `--preview` |
...
| | `export` / `import <bundle.zip>` | Export the workspace configuration as a bundle, or import one | `export`: `--include-issues`, `--include-notes`, `--template`, `--name`, `--out`; `import`: `--strategy` (`rename`, `merge`, `skip`), `--preview` |
| `pack` | `list/show/preview/install/installed/items/uninstall/download/preview-file/install-file/export` | Browse the pack catalogue, install, upgrade or uninstall a pack, upload your own, export the workspace as a pack | `list`: `--domain`; `install`/`install-file`: `--strategy`, `--force`; `export`: `--id`, `--version`, `--title`, `--summary`, `--domain`, `--metric-label`, `--metric-description`, `--include-issues`, `--include-notes`, `--out` |
| | `mcp list/add/update/remove` | Manage the workspace's MCP server library | Writes are owner/admin. A server added here is assigned to no agent — give it to one with `multica agent mcp add`. `list` shows names and transports only; the stored entry is write-only and never returned. `--server-config-file` / `--server-config-stdin` keep tokens out of shell history |
| | `member list/invite <email>` | View members, send invites | `invite`: `--role` (member or admin, default member) |
| | `halt` | Hold every agent in this workspace (owner/admin only) | `--reason` |
| | `resume` | Lift the hold on this workspace's agents (owner/admin only) | |
...
| | `label list/add/remove <skill-id>` | Manage labels on a skill | `add` / `remove`: `<label-id>` accepts a full UUID or short prefix |
...
| `triage` | `list` | List triage items (default: the pending queue) | `--pending`, `--state`, `--include-snoozed`, `--limit`, `--full-id` |
| | `verdict <item-id>` | Suggest accept or dismiss on a pending triage item (agents only) | `--accept` / `--dismiss`, `--reason` |
| `brain` | `list` | List workspace notes, optionally filtered by search text or tag | `--search`, `--tag`, `--archived`, `--limit`, `--full-id` |
| | `show <id>` | Print one workspace note | |
| | `save` | Save a durable piece of workspace knowledge as a note | `--title`, `--tags`, `--content` / `--content-file`, `--pinned`, `--id` |
| | `archive <id>` | Archive a note so it stops being injected into runs | |
| `wiki` | `start` | Start a wiki snapshot and announce the repository's file inventory | `--project`, `--resource`, `--commit`, `--paths-from` |
| | `page set` | Write one page into a wiki snapshot | `--project`, `--snapshot`, `--slug`, `--title`, `--content-file`, `--citations-file`, `--cite` (repeatable) |
| | `publish` | Publish a wiki snapshot, replacing the previous one in one step | `--project`, `--snapshot` |
| `criteria` | `list <issue-id>` | List an issue's acceptance criteria with their proof state | |
| | `set <issue-id>` | Replace the acceptance criteria (one `--text` per criterion) | `--text` (repeatable) |
| | `prove <issue-id> <criterion-id>` | Attach a proof to one criterion | `--type` (`test`, `file`, `screenshot`, `url`, `human_validation`), `--ref` |
| `decision` | `ask <issue-id>` | File a Decision Card: a question with options, a recommendation and an urgency | `--question`, `--option` (repeatable, `"id=Label"` or `"id=Label\|impact"`), `--recommend`, `--urgency` |
| `interview` | `ask <issue-id>` | File a requirement interview (up to three multiple-choice questions) from a JSON file or stdin | `--file` |
| `review` | `flag add` | Record one finding on a line range of a linked pull request | `--issue`, `--pr`, `--sha`, `--file`, `--line`, `--side`, `--severity`, `--confidence`, `--title`, `--body` |
| | `flag list` | List an issue's review flags, most severe first | `--issue`, `--state` |
| | `verdict` | Record this critic run's verdict (`pass`, `concerns`, `block`) on the change it reviewed | `--issue`, `--task`, `--verdict`, `--summary`, `--findings-file` |
...
| `runtime` | `list/usage/activity/update/rename/delete` | View and manage runtimes | `delete`: `--cascade` (unbinds bound agents, preserves their data, and cancels their runs) |
