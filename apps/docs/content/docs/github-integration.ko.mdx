...
| PR 자동 연결 | PR의 브랜치 이름과 제목에서 이슈 번호를 찾고, 본문에서는 종료 문구 바로 뒤에 있는 번호만 찾습니다. |
...
본문의 `Related to MUL-123` 같은 일반 언급은 해당 이슈에 전혀 연결되지 않습니다. Commit message와 PR 댓글도 연결을 트리거하지 않습니다.
...
2. 해당 이슈에 `Open` 또는 `Draft` 상태인 다른 연결 PR이 없습니다.
...
- **본문에 번호를 썼지만 연결되지 않음**: `Closes MUL-123`을 사용하거나 번호를 브랜치 이름 또는 PR 제목에 넣습니다.
