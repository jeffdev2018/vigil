...
## Ready for a first result?

The **Runtimes** page shows a short checklist when something still blocks a useful first agent result: machine online, agent CLI present, provider signed in (Claude/Codex), a linked repository when you need coding work, and a kicked-off Mika conversation for this member. It surfaces signals already on Runtimes and Settings; it does not replace onboarding. The checklist hides once every required step is ready. A repository remains optional for chatting with Mika. When no Claude or Codex runtime is online, CLI sign-in shows as not applicable rather than signed-in.
