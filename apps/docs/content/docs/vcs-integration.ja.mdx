...
description: ワークスペースごとにセルフホストの Forgejo、Gitea、GitLab インスタンスを接続し、ブランチ名かタイトルに課題番号を含む、または本文で終了キーワードの直後に課題番号を置いた Pull Request / Merge Request を対応する課題へ自動的に紐づけ、マージ時に「完了」へ移動して CI ステータスを表示します。
---
...
Multica はワークスペースごとにセルフホストの Git ホスティング、**Forgejo**、**Gitea**、**GitLab** のいずれかへ接続します。接続後、ブランチ名かタイトルに課題番号（例: `MUL-123`）を含む Pull Request（GitLab では Merge Request）、または本文で終了キーワードの直後に課題番号を置いた PR は、対応する[課題](/issues)へ**自動的に紐づけられ**、課題サイドバーの **Pull requests** に表示されます。終了キーワードを含む PR がマージされると、課題は「完了」へ移動します。head commit の CI はカード上のチェックバーとして表示されます。
...
- **課題の紐づけ** — タイトルまたはブランチの番号、および本文で終了キーワードの直後に置いた番号で PR を課題へ紐づけます。本文中の通常の参照では紐づきません。マージ済み PR に終了キーワード（`Closes`/`Fixes`/`Resolves MUL-123`）があり、open の関連 PR が残っていなければ、課題を「完了」へ移動します。
