...
description: ワークスペースごとにセルフホストの Forgejo、Gitea、GitLab インスタンスを接続し、ブランチ名かタイトルに課題番号を含む Pull Request / Merge Request を対応する課題へ自動的に紐づけ、紐づく PR がすべてマージされると「完了」へ移動し、CI ステータスを表示します。
---
...
Multica はワークスペースごとにセルフホストの Git ホスティング、**Forgejo**、**Gitea**、**GitLab** のいずれかへ接続します。接続後、ブランチ名かタイトルに課題番号（例: `MUL-123`）を含む Pull Request（GitLab では Merge Request）は、対応する[課題](/issues)へ**自動的に紐づけられ**、課題サイドバーの **Pull requests** に表示されます。課題に紐づく PR がすべてマージされると、課題は「完了」へ移動します。head commit の CI はカード上のチェックバーとして表示されます。
...
- **課題の紐づけ** — タイトルまたはブランチの番号で PR を課題へ紐づけます。本文は読み取らないため、本文の `Closes MUL-123` では紐づきません。課題に紐づく PR（GitHub とセルフホストの両方）がすべてマージされると、[GitHub](/github-integration) と同じワークスペース設定とルールに従って課題を「完了」へ移動します。
