...
// useIssueRealtime imports issueGoalKeys / followupKeys / recurrenceKeys from
// data/queries/*.ts, which also export api-calling query options and so import
// @/data/api at module scope. Under vitest that drags in workspace-store ->
// expo-secure-store -> react-native (Flow source rolldown cannot parse), and
// api.ts's module-level EXPO_PUBLIC_API_URL check throws. Same mock as
// data/realtime/issue-ws-updaters.test.ts; this suite only patches caches.
