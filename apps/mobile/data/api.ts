...
class ApiClient {
  ...
  /**
   * Durable human decisions addressed to this member. Fail closed: a malformed
   * page becomes null (never an empty list that looks like "nothing to do").
   * Mirrors packages/core/api/client.ts listIssueDecisions.
   */
  async listIssueDecisions(
    history = false,
    beforeId?: string,
    opts?: { signal?: AbortSignal },
  ): Promise<IssueDecisionPage | null> {
    const suffix = beforeId
      ? `&before_id=${encodeURIComponent(beforeId)}`
      : "";
    const raw = await this.fetch<unknown>(
      `/api/inbox/decisions?history=${history}${suffix}`,
      { signal: opts?.signal },
    );
    return parseWithFallback<IssueDecisionPage | null>(
      raw,
      IssueDecisionPageSchema,
      null,
      { endpoint: "GET /api/inbox/decisions" },
    );
  }
  ...
  async answerIssueDecision(
    ...
    answer: string,
  ): Promise<IssueDecision | null> {
    const raw = await this.fetch<unknown>(
      `/api/issues/${encodeURIComponent(issueId)}/decisions/${encodeURIComponent(decisionId)}/answer`,
      {
        method: "POST",
        body: JSON.stringify({ status, answer }),
      },
    );
    return parseWithFallback<IssueDecision | null>(
      raw,
      IssueDecisionSchema.refine(
        (row) =>
          row.id === decisionId &&
          row.issueId === issueId &&
          row.status === status &&
      ...
      {
        endpoint:
          "POST /api/issues/:id/decisions/:decisionID/answer",
      },
    );
  }

  async resumeIssueDecision(
    issueId: string,
    decisionId: string,
  ): Promise<IssueDecision | null> {
    const raw = await this.fetch<unknown>(
      `/api/issues/${encodeURIComponent(issueId)}/decisions/${encodeURIComponent(decisionId)}/resume`,
      { method: "POST" },
    );
    return parseWithFallback<IssueDecision | null>(
      raw,
      IssueDecisionSchema.refine(
        (row) =>
          ...
          row.issueId === issueId &&
          row.status === "answered" &&
          row.resumeTaskId !== null,
      ),
      null,
      {
        endpoint:
          "POST /api/issues/:id/decisions/:decisionID/resume",
      },
    );
  }
