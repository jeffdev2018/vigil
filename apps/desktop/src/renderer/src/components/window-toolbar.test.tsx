...
// The toolbar's labels come from the `layout` namespace, so it only renders
// them under an i18n provider. Rendering with the real EN bundle keeps the
// assertions on the shipped strings instead of a hand-written mock that could
// drift from the locale files.
function renderToolbar(node: ReactNode = <WindowToolbar />) {
  return render(
