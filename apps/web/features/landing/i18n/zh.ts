  ...
  changelog: {
    ...
    entries: [
      ...
      },
      {
        ...
        fixes: [
          "企业微信连续发多条消息时不再丢消息。",
          "给智能体换企业微信机器人后，旧机器人不会再留下记录。",
          "企业微信和钉钉群里引用的那条消息会一并交给智能体。",
          "钉钉的回复从第一条起就标明是哪个智能体在回答。",
          "已被撤销的群聊连接会显示为已断开。",
          "Grok、Pi、Copilot、Codex 的运行不再静默出错或漏掉部分回复。",
          "Cursor 的会话在连接超时后仍然保留，可以接着用。",
      ...
      },
      {
        version: "0.4.44",
        date: "2026-09-15",
        title: "钉钉引用回复、Issue 状态设置重做、运行用量更准、评论删除更安全",
        changes: [],
        features: [
          "钉钉群里的回复会引用你发出的那条消息。",
          "钉钉会用表情回应标出你的请求正在处理，完成后再标一次。",
          "钉钉的命令回执会保留你输入的内容，并关联它创建的 Issue。",
          "设置里的 Issue 状态按未开始、进行中、已完成、已关闭分组。",
          "拖动状态（包括内置状态）就能决定它在看板、列表和状态菜单里的位置。",
          "自定义状态可以自己选图标形状。",
          "装了 DeepSeek Harness Desktop 就会自动识别为运行时。",
          "自托管服务端每天发一次匿名部署概况，可用 DO_NOT_TRACK=1 关闭。",
        ],
        improvements: [
          "归档一个状态时会告诉你还有多少 Issue 在用它，并能直接跳过去。",
          "DeepSeek Harness 运行时起不来时，会说明缺了什么。",
          "命令行会显示每个 Autopilot 最近一次运行的结果。",
        ...
        fixes: [
          "删除一条评论会保留别人留在它下面的回复。",
          "已删除的回复不再留下占位，指向它的链接会落到旁边那条消息上。",
          "Issue 列表按状态排序会重新遵循你设定的顺序。",
          "你的智能体提交的 Issue 在等你处理时，会通知到你。",
          "两条评论同时派发同一个智能体会并入一次运行，不再失败。",
          "失败的委派任务会被重新接手，不再被落下。",
          "Claude、Codex、CodeBuddy、Qwen 等运行的用量不再多报。",
          "取消的任务不再计入智能体的成功率。",
          "Codex 的运行不再混入别的会话的回复。",
          "在 Windows 上打包的技能压缩包导入后可以正常使用。",
          "Hermes 会在任务实际运行的目录里找到你的本地技能。",
          "企业微信不再在文件前发一条空消息，文件没发出去时也会说明。",
          "飞书绑定二维码可以撑满一小时，不会在第一次检查时失效。",
          "旧版本的桌面端在服务端更新后仍会保留你的自定义状态。",
      ...
      },
      {
        ...
        improvements: [
          "智能体、技能、运行时的详情页在大屏下不再错位。",
          "大工作区里的 Issue 搜索更快了，多个词一起搜也不会卡住。",
          "收件箱列表默认更窄，给打开的内容多留出空间。",
          "自托管服务端可以设置搜索允许占用的内存上限。",
        ],
        fixes: [
          "你自己能手动触发的 Autopilot，现在智能体也能替你触发。",
          "每一次 Autopilot 改动都按背后那个人的权限来判断。",
          "Autopilot 的 Webhook 凭据不再出现在实时更新里。",
          "工作目录被清掉的 Pi 任务会改用新会话，不再反复失败。",
          "从频道新开的对话，标题取自你刚发出的那条指令。",
          "归档智能体会一并结束它的对话，Slack 里不再显示还在处理。",
          "普通成员可以用看不到的运行时上的智能体创建任务了。",
      ...
      },
      {
        ...
        features: [
          ...
          "要换成新的连接配置，现在是一个单独、明确的操作。",
          "保存 MCP 服务器时会直接定位到需要修改的那一项。",
        ],
        improvements: [
          "智能体的每一次执行，在各端和文档里统一叫「运行」。",
          "搜索结果返回更快了。",
        ],
        fixes: [
          "本机应用和服务端版本不一致时，任务仍然能正常启动。",
          "登录过期会回到登录页，上一个账号的数据也会清掉。",
          "离线不会再把你登出。",
      ...
      },
      {
        ...
        features: [
          ...
          "企业微信里的运行失败会回到你发起它的那个会话。",
          "桌面端可以用数字快捷键切换标签页。",
          "命令行可以按自定义属性筛选和排序 Issue。",
          "也能找出某个自定义属性还没填的 Issue。",
        ],
        improvements: [
          "把 Issue 分配给别人的步骤更少了，仍然可以先不启动。",
          "自托管服务器为保持运行时连接做的后台工作更少了。",
          "删除运行时会立刻生效，不会留下失效的连接。",
        ],
        fixes: [
          "回复 Squad 领队仍由该领队继续，交接信息和会话都不会丢。",
          "很长的 Issue 标识不会再遮住列表和表格里的标题。",
          "登录、报错和 404 页面会跟随你选择的语言。",
          "中途切换语言后，登录仍然可以正常完成。",
