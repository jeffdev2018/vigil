...
const countActiveTasksForIssues = `-- name: CountActiveTasksForIssues :one
SELECT COUNT(*)::bigint FROM agent_task_queue
WHERE issue_id = ANY($1::uuid[])
  AND status IN ('queued', 'dispatched', 'running', 'waiting_local_directory')
`

func (q *Queries) CountActiveTasksForIssues(ctx context.Context, issueIds []pgtype.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countActiveTasksForIssues, issueIds)
	var column_1 int64
...
INSERT INTO issue (
    ...
    now(), COALESCE($22::uuid, gen_random_uuid())
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, revision, last_activity_at, reopen_count, completed_at, duplicate_of_issue_id, contract_risk, contract_revision, goal_id, delegate_type, delegate_id, cycle_id, issue_type, recurrence_id, triage_state
`
...
WITH RECURSIVE tree AS (
    ...
    FROM issue c
    WHERE c.parent_issue_id = $1 AND c.workspace_id = $2
  UNION ALL
...
func (q *Queries) ListIssueDescendants(ctx context.Context, arg ListIssueDescendantsParams) ([]ListIssueDescendantsRow, error) {
	rows, err := q.db.Query(ctx, listIssueDescendants, arg.IssueID, arg.WorkspaceID, arg.MaxDepth)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) SetIssueContractRisk(ctx context.Context, arg SetIssueContractRiskParams) (Issue, error) {
	row := q.db.QueryRow(ctx, setIssueContractRisk, arg.ID, arg.WorkspaceID, arg.ContractRisk)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
WITH wakeup_source AS MATERIALIZED (SELECT set_config('multica.source_task_id', COALESCE($3::uuid::text, ''), true)), candidate AS MATERIALIZED (
    ...
    SELECT
        ...
        $16::text AS next_delegate_type,
        $17::uuid AS next_delegate_id,
