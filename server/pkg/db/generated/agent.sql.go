...
const countA2ARunsForIssueSince = `-- name: CountA2ARunsForIssueSince :one
...
type CountA2ARunsForIssueSinceParams struct {
	IssueID pgtype.UUID        `json:"issue_id"`
...
func (q *Queries) CountA2ARunsForIssueSince(ctx context.Context, arg CountA2ARunsForIssueSinceParams) (int64, error) {
	row := q.db.QueryRow(ctx, countA2ARunsForIssueSince, arg.IssueID, arg.Since)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
type CreateAgentParams struct {
	...
	RuntimeRouting           interface{} `json:"runtime_routing"`
}
...
type CreateAgentTaskParams struct {
	...
	Routing              []byte        `json:"routing"`
	A2aDepth             pgtype.Int4   `json:"a2a_depth"`
	RunGroupID           pgtype.UUID   `json:"run_group_id"`
	ModelOverride        pgtype.Text   `json:"model_override"`
	ID                   pgtype.UUID   `json:"id"`
}
...
SELECT
    ...
    COALESCE($7::jsonb, p.failover_history),
...
type CreateRetryTaskParams struct {
	...
	RuntimeConnectedApps []byte             `json:"runtime_connected_apps"`
	FailoverHistory      []byte             `json:"failover_history"`
	CheckpointAttempts   pgtype.Int4        `json:"checkpoint_attempts"`
	NewTaskID            pgtype.UUID        `json:"new_task_id"`
}
...
const deleteAgentTasksByID = `-- name: DeleteAgentTasksByID :exec
...
func (q *Queries) DeleteAgentTasksByID(ctx context.Context, ids []pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteAgentTasksByID, ids)
	return err
}
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, last_activity_at, permission_profile_id, failover_history, routing_decision, pause_requested_at, resumed_by_task_id, last_checkpoint_seq, checkpoint_attempts, checkpointed_at, touched_paths, drift_reason, preempted_at, preempted_by_task_id, review_of_task_id, task_class, routing, safe_mode, model_key_id, confidence, memory_context, leg_role, workflow_root_task_id, dispatch_lane, checkpoint_sha, turn_seq, a2a_depth, run_group_id, model_override, diff_stat, diff_unified FROM agent_task_queue
WHERE id = $1
`
...
type GetTaskDispatchLanesRow struct {
	ID           pgtype.UUID `json:"id"`
	DispatchLane string      `json:"dispatch_lane"`
}
...
func (q *Queries) GetTaskDispatchLanes(ctx context.Context, taskIds []pgtype.UUID) ([]GetTaskDispatchLanesRow, error) {
	rows, err := q.db.Query(ctx, getTaskDispatchLanes, taskIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetTaskDispatchLanesRow{}
	for rows.Next() {
		var i GetTaskDispatchLanesRow
		if err := rows.Scan(&i.ID, &i.DispatchLane); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const listTaskTurnsAfter = `-- name: ListTaskTurnsAfter :many
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, last_activity_at, permission_profile_id, failover_history, routing_decision, pause_requested_at, resumed_by_task_id, last_checkpoint_seq, checkpoint_attempts, checkpointed_at, touched_paths, drift_reason, preempted_at, preempted_by_task_id, review_of_task_id, task_class, routing, safe_mode, model_key_id, confidence, memory_context, leg_role, workflow_root_task_id, dispatch_lane, checkpoint_sha, turn_seq, a2a_depth, run_group_id, model_override, diff_stat, diff_unified FROM agent_task_queue
WHERE turn_seq IS NOT NULL
  AND turn_seq > $1::int
  AND (
      ($2::uuid IS NOT NULL AND issue_id = $2::uuid)
      OR ($3::uuid IS NOT NULL AND chat_session_id = $3::uuid)
  )
ORDER BY turn_seq DESC
`

type ListTaskTurnsAfterParams struct {
	AfterTurnSeq  int32       `json:"after_turn_seq"`
	IssueID       pgtype.UUID `json:"issue_id"`
	ChatSessionID pgtype.UUID `json:"chat_session_id"`
}
...
func (q *Queries) ListTaskTurnsAfter(ctx context.Context, arg ListTaskTurnsAfterParams) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listTaskTurnsAfter, arg.AfterTurnSeq, arg.IssueID, arg.ChatSessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const recordTaskTurnCheckpoint = `-- name: RecordTaskTurnCheckpoint :one
UPDATE agent_task_queue
...
WHERE agent_task_queue.id = $2 AND agent_task_queue.checkpoint_sha IS NULL
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, last_activity_at, permission_profile_id, failover_history, routing_decision, pause_requested_at, resumed_by_task_id, last_checkpoint_seq, checkpoint_attempts, checkpointed_at, touched_paths, drift_reason, preempted_at, preempted_by_task_id, review_of_task_id, task_class, routing, safe_mode, model_key_id, confidence, memory_context, leg_role, workflow_root_task_id, dispatch_lane, checkpoint_sha, turn_seq, a2a_depth, run_group_id, model_override, diff_stat, diff_unified
`

type RecordTaskTurnCheckpointParams struct {
	CheckpointSha pgtype.Text `json:"checkpoint_sha"`
	ID            pgtype.UUID `json:"id"`
}
...
func (q *Queries) RecordTaskTurnCheckpoint(ctx context.Context, arg RecordTaskTurnCheckpointParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, recordTaskTurnCheckpoint, arg.CheckpointSha, arg.ID)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
const stampTaskDispatchLane = `-- name: StampTaskDispatchLane :exec
UPDATE agent_task_queue
SET dispatch_lane = $1
WHERE id = $2
  AND status = 'queued'
...
type StampTaskDispatchLaneParams struct {
	DispatchLane string      `json:"dispatch_lane"`
	TaskID       pgtype.UUID `json:"task_id"`
}
...
func (q *Queries) StampTaskDispatchLane(ctx context.Context, arg StampTaskDispatchLaneParams) error {
	_, err := q.db.Exec(ctx, stampTaskDispatchLane, arg.DispatchLane, arg.TaskID)
	return err
}
...
const touchAgentTaskActivity = `-- name: TouchAgentTaskActivity :exec
UPDATE agent_task_queue
...
func (q *Queries) TouchAgentTaskActivity(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, touchAgentTaskActivity, id)
	return err
}
...
UPDATE agent SET
    ...
    runtime_routing = COALESCE($8, runtime_routing),
...
type UpdateAgentParams struct {
	...
	RuntimeID                pgtype.UUID `json:"runtime_id"`
	RuntimeRouting           pgtype.Text `json:"runtime_routing"`
