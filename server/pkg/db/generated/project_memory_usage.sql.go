...
// versions:
//   sqlc v1.31.1
// source: project_memory_usage.sql
...
const getProjectMemoryUsage = `-- name: GetProjectMemoryUsage :one
...
type GetProjectMemoryUsageParams struct {
...
type GetProjectMemoryUsageRow struct {
...
func (q *Queries) GetProjectMemoryUsage(ctx context.Context, arg GetProjectMemoryUsageParams) (GetProjectMemoryUsageRow, error) {
	row := q.db.QueryRow(ctx, getProjectMemoryUsage,
		arg.ProjectID,
		arg.WorkspaceID,
		arg.Since,
		arg.Until,
	)
	var i GetProjectMemoryUsageRow
	err := row.Scan(
		&i.StartedRuns,
		&i.RecordedRuns,
		&i.UnrecordedRuns,
		&i.RunsWithProjectMemory,
		&i.Versions,
	)
	return i, err
}
