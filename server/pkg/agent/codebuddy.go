...
//
// buildEnv's foreign-vendor-key scrub keys off Config.provider, and codebuddy
// is not tied to a model vendor (modelkey.VendorForRuntime("codebuddy") is
// empty), so the task env alone is what the child needs here.
func buildCodebuddyEnv(extra map[string]string) []string {
	return append(buildEnv(Config{Env: extra}), codebuddyDisableBackgroundTasksEnv+"=1")
}
