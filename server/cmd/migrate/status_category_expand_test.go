...
func TestStatusCategoryExpandUpgradePaths(t *testing.T) {
	for _, applied := range []bool{false, true} {
      ...
      VALUES ('00000000-0000-0000-0000-000000000001',$1,'System',$1,'#123456',true,NULL),
			...
			opts := f.opts()
			opts.Files = realMigrationFiles(t, []string{"954_issue_status_lifecycle_categories"}, "up")
			if applied {
			...
			opts.Files = realMigrationFiles(t, []string{"954_issue_status_lifecycle_categories", "963_issue_status_category_expand"}, "up")
			...
			if !applied {
				...
				assertMigrationVersionRecorded(t, ctx, pool, f.schema, "963_issue_status_category_expand", false)
			...
			execFile("963_issue_status_category_expand")
			var after string
      ...
      UPDATE issue_status SET category = 'closed' WHERE key = 'custom_cancelled'`); err != nil {
			...
			assertMigrationVersionRecorded(t, ctx, pool, f.schema, "954_issue_status_lifecycle_categories", true)
			assertMigrationVersionRecorded(t, ctx, pool, f.schema, "963_issue_status_category_expand", true)
		})
	}
}
