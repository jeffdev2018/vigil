...
func TestTaskSupplementMigrationsUpDownUpInIsolatedSchema(t *testing.T) {
	...
	for _, direction := range []string{"up", "down", "up"} {
		versions := []string{
			"703_task_supplement",
			"704_task_supplement_request_index",
			"705_task_supplement_capability_index",
			"706_task_supplement_comment_index",
			"707_task_supplement_primary_key",
			"708_task_supplement_teardown_guard",
			"709_task_supplement_application_settlement",
		}
		if direction == "down" {
