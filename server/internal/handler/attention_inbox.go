...
func (h *Handler) notifyDecisionRequested(ctx context.Context, issue db.Issue, decision db.IssueDecision, actorType, actorID string) {
	...
	memberIDs := make([]pgtype.UUID, 0, len(recipients))
	for _, rcpt := range recipients {
		if rcpt.Type == "member" {
			memberIDs = append(memberIDs, rcpt.ID)
		}
	}
	// Vigil learns you (K71): a recipient's learned rule may settle the card
	// before anyone is asked; the auto-decision is notified on its own.
	if h.autoDecide(ctx, issue, decision, memberIDs) {
		return
	}
	defer func() {
