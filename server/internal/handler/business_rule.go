...
Reply with JSON only: {"title":"<short rule title>","predicate":{"all":[{"field":"<field>","op":"<op>","value":<value>}],"any":[...]}}.
Operators: eq, ne, lt, lte, gt, gte for numbers; eq, ne for booleans and strings; in with a list for strings. Use only the fields listed; if the rule cannot be expressed with them, reply {"title":"","predicate":{},"error":"<why>"}.`
...
func (h *Handler) compileBusinessRule(ctx context.Context, attach, text string) (title string, predicate []byte, err error) {
	...
	if err := json.Unmarshal([]byte(raw), &out); err != nil {
		return "", nil, fmt.Errorf("malformed compilation: %w", err)
	}
	if out.Error != "" {
		return "", nil, fmt.Errorf("rule cannot be expressed: %s", out.Error)
	}
	return strings.TrimSpace(out.Title), out.Predicate, nil
}
...
func (h *Handler) CreateBusinessRule(w http.ResponseWriter, r *http.Request) {
	...
	title, predicate := strings.TrimSpace(req.Title), []byte(req.Predicate)
	if len(predicate) == 0 {
		...
		var err error
		title, predicate, err = h.compileBusinessRule(r.Context(), req.AttachPoint, req.NaturalLanguage)
		if err != nil {
	...
	rule, err := h.Queries.CreateBusinessRule(r.Context(), db.CreateBusinessRuleParams{
		...
		CompiledPredicate: canonical, AttachPoint: req.AttachPoint, CreatedBy: parseUUID(userID),
	})
	if err != nil {
