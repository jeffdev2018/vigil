...
func TestIssueStatusLifecycleMigrationPreservesIdentity(t *testing.T) {
	...
	migration, err := os.ReadFile("../../migrations/954_issue_status_lifecycle_categories.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	iconMigration, err := os.ReadFile("../../migrations/955_issue_status_icon.up.sql")
	if err != nil {
		t.Fatal(err)
	}
