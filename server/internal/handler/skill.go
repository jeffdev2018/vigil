...
type SkillResponse struct {
	...
	// PluginInstallationID, when set, means this skill is generated by that
	// plugin installation and cannot be edited directly through this API (see
	// refusePluginManagedSkill) — the client should grey out edit affordances
...
func skillToResponse(s db.Skill) SkillResponse {
	return SkillResponse{
		...
		Status:               s.Status,
		PluginInstallationID: uuidToPtr(s.PluginInstallationID),
	}
}
...
// refusePluginManagedSkill refuses a write to a skill a plugin installation
// owns. Deliberately separate from canManageSkill: that check is about WHO may
// edit a skill (creator vs admin), this one is about WHETHER the skill is
// editable through this endpoint at all. A plugin's skill is a generated
// artifact of its installation — a manifest re-fetch or an upgrade would
// silently discard an edit made here, so the edit is refused up front instead
// of accepted and then lost. Uninstalling (or upgrading) the plugin is the
// supported way to change or remove it.
func (h *Handler) refusePluginManagedSkill(w http.ResponseWriter, skill db.Skill) bool {
...
func (h *Handler) UpdateSkill(w http.ResponseWriter, r *http.Request) {
	...
	if h.refusePluginManagedSkill(w, skill) {
		return
	}
...
func skillImportOverwriteFailure(err error) (int, string) {
	...
	case errors.Is(err, errSkillOverwritePluginManaged):
		return http.StatusConflict, "this skill is managed by a plugin and cannot be edited directly; uninstall or upgrade the plugin instead"
	default:
