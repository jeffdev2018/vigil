...
func TestTaskMessageCallIDLiveAndHistory(t *testing.T) {
	...
	// The daemon reader (ListTaskMessages) answers a bare array for reconnect
	// catch-up; the user reader (ListTaskMessagesByUser) answers the
	// TaskActivityResponse envelope (messages + the run's action lane), so
	// each declares its own response shape here rather than the handler
	// changing to please one decoder.
	for _, reader := range []struct {
		name     string
		handler  http.HandlerFunc
		envelope bool
	}{
		{"daemon", h.ListTaskMessages, false},
		{"user", h.ListTaskMessagesByUser, true},
	} {
		for _, suffix := range []string{"", "?since=2"} {
			t.Run(reader.name+suffix, func(t *testing.T) {
				...
				resp := testutil.Call(t, reader.handler, req.WithContext(ctx)).Want(http.StatusOK)
