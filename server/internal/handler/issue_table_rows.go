...
func (h *Handler) ListIssueTableRows(w http.ResponseWriter, r *http.Request) {
  ...
  LIMIT %s
	...
	// duplicate_of_issue_id is not displayed, but appendDuplicateOriginal below
	// reads it to resolve the original's status category for a row marked
	// duplicate. Leaving it out of the select left that field zero on every
	// row, so the resolution silently never happened.
	query := fmt.Sprintf(`%s
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority,
       ...
       i.parent_issue_id, i.position, i.start_date, i.due_date, i.created_at,
	       ...
	       i.revision, i.issue_type, i.duplicate_of_issue_id,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&row.issue.DelegateType,
			&row.issue.DelegateID,
