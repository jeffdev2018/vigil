...
func TestListIssuesSortsByStatusAndUpdatedAt(t *testing.T) {
	...
	t.Cleanup(func() {
		// The issues below are raw inserts and there is no FK cascade by house
		// rule, so dropping the project alone leaves them behind. They carry
		// this test's custom status, whose catalog row is deleted with it, and a
		// later test in this package exporting the shared workspace then sees an
		// issue whose status no longer exists.
