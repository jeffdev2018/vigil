...
// autoCompletePR is a PR resolved from either provider table.
type autoCompletePR struct {
	ID       pgtype.UUID
...
func (h *Handler) findPullRequestForLink(ctx context.Context, workspaceID pgtype.UUID, req LinkIssuePullRequestRequest) (autoCompletePR, bool, error) {
	if req.PullRequestID != "" {
		id, err := util.ParseUUID(req.PullRequestID)
		if err != nil {
			return autoCompletePR{}, false, nil
		}
	...
	if pr, err := h.Queries.FindGitHubPullRequestByURL(ctx, db.FindGitHubPullRequestByURLParams{WorkspaceID: workspaceID, HtmlUrl: normalized}); err == nil {
		return autoCompletePR{ID: pr.ID, Provider: "github", Number: pr.PrNumber, github: true}, true, nil
	} else if !errors.Is(err, pgx.ErrNoRows) {
		return autoCompletePR{}, false, err
	}
	if pr, err := h.Queries.FindVCSPullRequestByURL(ctx, db.FindVCSPullRequestByURLParams{WorkspaceID: workspaceID, HtmlUrl: normalized}); err == nil {
		return autoCompletePR{ID: pr.ID, Provider: pr.Provider, Number: pr.PrNumber}, true, nil
	} else if !errors.Is(err, pgx.ErrNoRows) {
		return autoCompletePR{}, false, err
	}
	return autoCompletePR{}, false, nil
...
func (h *Handler) findPullRequestByID(ctx context.Context, workspaceID, id pgtype.UUID) (autoCompletePR, bool, error) {
...
func (h *Handler) SetIssuePRAutoComplete(w http.ResponseWriter, r *http.Request) {
	...
	if before != *req.Disabled {
		h.recordPRAutoCompleteActivity(r.Context(), issue, actorType, actorID, *req.Disabled, h.actingTaskID(r))
	}
	h.publishIssuePullRequestsChanged(workspaceID, issue.ID)
	h.ListPullRequestsForIssue(w, r)
}

// recordPRAutoCompleteActivity logs the toggle. resolveActor returns an agent
// actor whenever the request carries a task token, so this can be a run
// changing the setting on its own issue; taskID stamps details.task_id
// (only when valid) so the change still surfaces in that run's action lane.
func (h *Handler) recordPRAutoCompleteActivity(ctx context.Context, issue db.Issue, actorType, actorID string, disabled bool, taskID pgtype.UUID) {
	detailsMap := map[string]any{"disabled": disabled}
	if taskID.Valid {
		detailsMap["task_id"] = util.UUIDToString(taskID)
	}
