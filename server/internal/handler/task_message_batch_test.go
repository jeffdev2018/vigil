...
func TestReportTaskMessagesFallsBackWholeBatchForClockSkew(t *testing.T) {
	...
	// The window is compared against a timestamp PostgreSQL stamps, so it has
	// to tolerate host/container clock skew: the assertion is "the database
	// fallback was used", and the rejected client value is a minute out, far
	// outside this margin.
	const clockSkewMargin = 2 * time.Second
	fallbackBefore := time.Now().UTC().Add(-clockSkewMargin)
	...
	fallbackAfter := time.Now().UTC().Add(clockSkewMargin)
