...
func (s *RedisCliAuthStore) load(ctx context.Context, id string) (*CliAuthRequest, error) {
	...
	// Expiry is evaluated on every read and mutation. A reader must not write
	// its stale snapshot over a concurrent terminal report.
	applyCliAuthTimeout(&req, time.Now())
	return &req, nil
}

var claimCliAuthScript = redis.NewScript(`
...
func (s *RedisCliAuthStore) PopPending(ctx context.Context, runtimeID string) (*CliAuthRequest, error) {
	...
	for attempt := 0; attempt < cliAuthRedisPopMaxRetries; attempt++ {
		...
		claimed, err := claimCliAuthScript.Run(ctx, s.rdb,
...
func (s *RedisCliAuthStore) update(ctx context.Context, id string, fn func(*CliAuthRequest)) error {
	...
	for attempt := 0; attempt < 8; attempt++ {
		err := s.rdb.Watch(ctx, func(tx *redis.Tx) error {
			...
			now := time.Now()
			if !applyCliAuthTimeout(&req, now) {
				fn(&req)
				req.UpdatedAt = now
			}
			data, err := json.Marshal(&req)
			...
			_, err = tx.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
				pipe.Set(ctx, key, data, cliAuthStoreRetention)
				return nil
			})
			return err
		}, key)
	...
	return fmt.Errorf("CLI authentication request changed concurrently; retry report")
}
