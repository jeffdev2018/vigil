...
// taskStatusResponse is the /status body. Upstream returns status alone; this
// fork adds the K19 pause signal the daemon stops on.
...
func TestGetTaskStatus_DoesNotResolveSourceWorkspace(t *testing.T) {
	...
	for _, tc := range []struct{ query, column, id string }{
		...
		t.Run(tc.query, func(t *testing.T) {
			...
			// This fork also reports the K19 pause signal, so the body is not a
			// map[string]string the way it is upstream.
