...
func TestRunPollingRecoversPanicAndFailsSession(t *testing.T) {
	...
	wsID := uuidFromStringSvc(t, "11111111-1111-1111-1111-111111111111")
	...
	// The terminal outcome is written to the session store, so the record has
	// to exist before polling runs.
	if err := s.sessionStore.Create(context.Background(), InstallSessionState{
		ID: sess.id, WorkspaceID: wsID, Status: RegistrationStatusPending, ExpiresAt: sess.expiresAt,
	}, time.Minute); err != nil {
		t.Fatalf("create session: %v", err)
	}
	s.runPolling(sess)
	state, err := s.sessionStore.Get(context.Background(), wsID, sess.id)
	if err != nil {
