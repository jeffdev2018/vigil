...
func (StatusCategory) Preflight(ctx context.Context, tx pgx.Tx) error {
	var applied bool
	if err := tx.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE version='963_issue_status_category_expand')`).Scan(&applied); err != nil {
		return err
	}
	if !applied {
		return fmt.Errorf("migration 963_issue_status_category_expand is not applied")
	}
	var compatible bool
