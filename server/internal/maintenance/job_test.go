...
func fixture(t *testing.T) (*pgxpool.Pool, *Service) {
	...
	for _, name := range []string{"332_issue_status", "333_issue_status_pkey_index", "963_issue_status_category_expand", "970_maintenance_job", "971_maintenance_job_id_index", "972_maintenance_job_idempotency_index", "973_maintenance_job_active_index"} {
	...
	exec("INSERT INTO schema_migrations VALUES ('963_issue_status_category_expand')")
...
func TestPreflightChecksActualSchemaAndLedger(t *testing.T) {
	...
	if _, err := s.Create(ctx, request(true)); err == nil {
		t.Fatal("missing 963_issue_status_category_expand accepted")
	}
	if _, err := pool.Exec(ctx, "INSERT INTO schema_migrations VALUES ('963_issue_status_category_expand')"); err != nil {
		t.Fatal(err)
	}
...
func TestUnknownStoredDataCannotComplete(t *testing.T) {
 ...
 UPDATE issue_status SET category='unexpected' WHERE id='00000000-0000-0000-0000-000000000008'`); err != nil {
	...
	sql, err := os.ReadFile("../../migrations/963_issue_status_category_expand.up.sql")
	if err != nil {
		t.Fatal(err)
	}
...
func TestContractPreservesAuditAndRejectsCategoryProcessor(t *testing.T) {
	...
	for _, name := range []string{"976_issue_status_category_backfill", "977_issue_status_category_contract", "978_issue_status_category_validate", "979_issue_status_category_read_contract"} {
		body, err := os.ReadFile(filepath.Join("..", "..", "migrations", name+".up.sql"))
		if err != nil {
			t.Fatal(err)
		}
