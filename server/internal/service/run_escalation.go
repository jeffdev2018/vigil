...
func (s *TaskService) escalateRunToStrongerRuntime(ctx context.Context, issue db.Issue, agent db.Agent, task db.AgentTaskQueue, conf TaskConfidence, cfg ConfidenceReview) bool {
	...
	// Bounded workflows (JEF-275): the escalation grows the scored run's
	// workflow, so it is refused once that workflow reached its ceiling. The
	// two caps answer different questions — max_escalations bounds THIS
	// cascade chain, the workflow ceiling bounds every leg the run's workflow
	// already spent on reviews, revisions and retries — so both must hold. The
	// refusal is audited inside WorkflowAllowsLeg and falls through to the
	// JEF-240 human-review path, which is the right outcome: the workflow may
	// not grow, so a human takes it from here.
	if allowed, reason := s.WorkflowAllowsLeg(ctx, task, LegRoleEscalation); !allowed {
		slog.Info("run confidence cascade: "+WorkflowLegRefusedNote(LegRoleEscalation, reason),
			"task_id", util.UUIDToString(task.ID), "reason", reason)
		return false
	}
	...
	// Per-leg accounting (JEF-274): the escalation is a leg of the scored
	// run's workflow, hanging off the run it escalated from. Best-effort, like
	// the rerun stamp — losing the leg record must not lose the escalation.
	// Deliberately after the coalescing check above: a merged task belongs to
	// its own lineage and must not be restamped into this workflow.
	if stamped, serr := s.StampLeg(ctx, newTask, LegRoleEscalation, task); serr != nil {
		slog.Warn("run confidence cascade: leg stamp failed",
			"task_id", util.UUIDToString(newTask.ID), "error", serr)
	} else {
		newTask = stamped
	}
