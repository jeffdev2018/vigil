...
func classifyOrigin(issue db.Issue, opts IssueCreateOpts) (source, taskID, autopilotRunID string) {
	...
	case "epic":
		...
		return analytics.SourceManual, "", ""
	case "email", "twenty":
		// Accepted from the triage queue: an inbound email (610) or a Twenty
		// CRM event (OS plan, chantier 2). A human accepted the item, so the
		// manual source label; origin_id is the queue item.
		return analytics.SourceManual, "", ""
	case recurrenceOriginType:
		// Spawned by a recurrence rule a member set (origin_id = the rule).
		// The standing order is human, so the manual source label.
		return analytics.SourceManual, "", ""
