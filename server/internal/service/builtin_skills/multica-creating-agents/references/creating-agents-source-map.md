...
| `thinking-level` / `service-tier` flags on `agent update` | 189–190 | Thin pass-throughs; `service-tier` repeats the empty / `default` / catalog-tier contract, and an explicit empty string clears the saved override and restores the runtime/local Codex default | `multica agent update --help` |
