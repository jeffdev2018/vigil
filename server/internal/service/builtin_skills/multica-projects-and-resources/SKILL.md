...
A project's `description` is also durable context: when an issue (or a quick-create task) is bound to a project, the project description is injected into the agent's brief under `## Project Context` and written to `.multica/project/resources.json` as `project_description`. Use it for project background. Shared operating rules belong in **Project memory**, published by a workspace owner/admin in the project sidebar. Up to 20 human-reviewed rules are added to the claim context with their revision for project-linked issues, chats, automations, and quick creation. Machine credentials cannot publish these rules; propose a correction to a human instead of bypassing review by editing the description. From a delivery **changes requested** review on a project-linked issue, an owner/admin can also **Promote to project memory** — the review evidence is stored as `source_review` on that publication (one promotion per review; retries are idempotent). The project UI also shows **Memory across runs**: how often each published revision was prepared into claim context over the last 30 days (not proof the model followed it).

Each publication preserves a version. Owners/admins can inspect history and restore a prior version as a new publication, subject to the current revision check. Restoration keeps the original expiration and any attached correction provenance; it never silently renews an expired rule set. Optional expiration excludes the rules from newly claimed runs after that timestamp. Runs already started retain their original context. History is scoped to the project and removed when the project or workspace is deleted.
