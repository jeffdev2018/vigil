...
// "pinned" and checks that claim with statusWriterPinPattern — a regex proving
// the pin line is still written. That catches the pin being deleted; it cannot
// catch the pin being bypassed, because a second path in the same file, or a
// status that reaches params from the tool's arguments, would leave the matched
// line untouched.
//
// So the claim is also checked where it matters, against the database: the
// model asks for a status change outright, and the row must ignore it while
// still applying the edits the tool does own. If that ever stops holding, a
// native agent can move an issue without meeting the F28 transition gate.
