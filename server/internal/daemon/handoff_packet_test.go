...
func TestPromptRendersHandoffPacket(t *testing.T) {
	t.Parallel()
	task := Task{ID: "t", IssueID: "MUL-1", HandoffNote: "scope: only the API", HandoffPacket: &HandoffPacket{
	...
	for _, want := range []string{"handoff note", "scope: only the API", "handoff packet", "- Objective: Ship the fix", "  - keep the table", "Failed attempts", "dropping it broke tests", "- Next action: open the PR", "POST /api/issues/{issue}/handoff-packet"} {
