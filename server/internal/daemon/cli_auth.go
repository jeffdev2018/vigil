...
func (d *Daemon) handleCliAuth(ctx context.Context, rt Runtime, pending PendingCliAuth) {
	...
	if active, loaded := d.cliAuthActive.LoadOrStore(rt.Provider, pending.ID); loaded {
		if active != pending.ID {
			d.reportCliAuthFailure(ctx, rt, pending.ID, fmt.Errorf("another authentication is in progress on this machine; retry when it finishes"))
		}
		return
	}
	defer d.cliAuthActive.Delete(rt.Provider)
