...
function cliAuthStatus(
  ...
  // A definite "signed out" outranks a runtime we simply have not heard from:
  // the member has something to fix either way, and naming it is more useful.
  return sawUnauthenticated ? "blocked" : "unknown";
}
...
export function deriveActivationReadiness(
  ...
  const cliStatus: ActivationStepStatus =
    online.length > 0 || localCliPresent ? "ready" : "blocked";
