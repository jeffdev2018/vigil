...
export const AgentTaskSchema = z.object({
  ...
  // Hold text is gated server-side to waiting_local_directory; treat anything
  // else (or a blank) as absent so an older client never shows a stale hold
  // explanation on a running row that somehow still carries the column.
...
export const RuntimeCliAuthRequestSchema = z.object({
  ...
  action: z.string(),
  status: z.string(),
  verification_url: z.url().refine((value) => {
...
}).loose().refine((value) => value.status !== "completed" || typeof value.authenticated === "boolean");

export const MALFORMED_RUNTIME_CLI_AUTH_REQUEST: RuntimeCliAuthRequest = {
  id: "",
  runtime_id: "",
  action: "unknown",
  status: "failed",
...
export const AgentMemoryEvaluationSchema = z.object({
  ...
  // Optional for older backends; when present must be the server SHA-256 receipt.
...
// Human decisions fail closed: an incomplete response is never approval or a
// successful resume. An unknown future status must be reviewed by a newer client.
export const IssueDecisionSchema = z.object({
  id: z.string().uuid(), issue_id: z.string().uuid(), agent_id: z.string().uuid(),
  source_task_id: z.string().uuid(), recipient_id: z.string().uuid(), requested_by: z.string().uuid(),
  requester_type: z.enum(["member", "agent"]), question: z.string().min(1), context: z.string(),
  options: z.array(z.string().min(1)).max(8), status: z.enum(["open", "answered", "cancelled"]),
  answer: z.string().min(1).nullable(), answered_by: z.string().uuid().nullable(),
  answered_at: z.string().datetime({ offset: true }).nullable(), resume_task_id: z.string().uuid().nullable(),
  created_at: z.string().datetime({ offset: true }),
}).refine((row) => {
  if (row.status === "open") return row.answer === null && row.answered_by === null && row.answered_at === null && row.resume_task_id === null;
  if (row.answered_by !== row.recipient_id || row.answered_at === null) return false;
  if (row.status === "cancelled") return row.answer === null && row.resume_task_id === null;
  return row.answer !== null;
}).transform((row) => ({
  id: row.id, issueId: row.issue_id, agentId: row.agent_id, sourceTaskId: row.source_task_id,
  recipientId: row.recipient_id, requestedBy: row.requested_by, requesterType: row.requester_type,
  question: row.question, context: row.context, options: row.options, status: row.status, answer: row.answer,
  answeredBy: row.answered_by, answeredAt: row.answered_at, resumeTaskId: row.resume_task_id, createdAt: row.created_at,
}));
export type IssueDecision = z.infer<typeof IssueDecisionSchema>;
export const IssueDecisionPageSchema = z.object({ decisions: z.array(IssueDecisionSchema), next_before_id: z.string().uuid().nullable() })
  .transform((row) => ({ decisions: row.decisions, nextBeforeId: row.next_before_id }));
export type IssueDecisionPage = z.infer<typeof IssueDecisionPageSchema>;

export const MemoryExecutionConfigSchema = z.object({ runtime_id: z.string().uuid(), provider: z.string(), model: z.string().min(1), effort: z.string(), config_hash: z.string().regex(/^[a-f0-9]{64}$/), max_cases: z.number().int().min(2).max(8), timeout_seconds: z.number().int().positive().max(60), check_modes: z.array(z.enum(["exact", "json", "javascript"])).catch(["exact"]).default(["exact"]) });
