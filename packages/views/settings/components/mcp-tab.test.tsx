...
vi.mock("@multica/core/workspace/queries", () => ({
  workspaceMcpServersOptions: () => ({
    queryKey: ["workspaces", "workspace-1", "mcp-servers"],
  }),
}));
...
describe("McpTab", () => {
  ...
  it("shows each transport as visible text beside its labeled icon", async () => {
    ...
    expect(
      await screen.findByText("Replace configuration", { exact: true }),
    ).toBeVisible();
  });
  ...
  it.each([
    ...
    "provides a connection example when replacing %s",
    async (name, label, example) => {
      const user = userEvent.setup();
      render(<McpTab />, { wrapper: Wrapper });
  ...
  it("opens a new shared dialog without displaying validation errors", async () => {
    ...
    expect(screen.getByLabelText("Server name")).not.toHaveAttribute(
      "aria-invalid",
    );
    expect(screen.getByLabelText("Command")).not.toHaveAttribute(
      "aria-invalid",
    );
  ...
  it.each([
    ...
    "rejects the server name %s with its precise inline error",
    async (name, message) => {
      const user = userEvent.setup();
      render(<McpTab />, { wrapper: Wrapper });
  ...
  it.each([
    ...
    {
      reason: "an empty error",
      error: new Error(""),
      message: "Could not rename the shared MCP server",
    },
    {
      reason: "a non-Error rejection",
      error: null,
      message: "Could not rename the shared MCP server",
    },
  ...
  it("blocks competing library actions while an inline rename is pending", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("textbox", { name: "Rename server" }),
      ).toBeNull(),
    );
  });
  ...
  it.each(["sse", "websocket"])(
    ...
    async (transport) => {
      ...
      expect(
        screen.getByRole("tab", { name: "Visual editor" }),
