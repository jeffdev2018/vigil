              ...
              {projects.map((p) => {
                ...
                return (
                  <label key={p.id} className="flex items-center gap-2 text-caption">
                    <Checkbox checked={out} disabled={disabled || !draft.enabled} onCheckedChange={(v) => persist({ ...draft, opt_out_project_ids: v === true ? [...draft.opt_out_project_ids, p.id] : draft.opt_out_project_ids.filter((id) => id !== p.id) })} />
                    <span>{p.title}</span>
                  </label>
                );
              })}
