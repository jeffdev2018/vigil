          ...
          <div className="mx-auto" style={{ width: layout.width * zoom, height: layout.height * zoom }}>
            <div className="relative origin-top-left" style={{ width: layout.width, height: layout.height, transform: `scale(${zoom})` }}>
              ...
              {layout.nodes.map(({ unit: u, x, y }) => {
                ...
                return <button key={u.id} type="button" data-testid="org-node" aria-pressed={selected === u.id} onClick={() => { selectTeam(u.id, "connections"); setAddingPeople(false); setPicked([]); }} className={cn("group absolute flex h-[178px] w-[280px] flex-col overflow-hidden rounded-2xl border bg-card text-left shadow-surface transition-[box-shadow,border-color] duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", selected === u.id ? "border-info shadow-menu ring-4 ring-info/10" : "hover:border-info/50 hover:shadow-menu")} style={{ left: x, top: y }}>
