...
// Only the leaf messages: the MCP blocks also carry nested groups (the K77
// tool catalogue lives at `settings:mcp.tools`), and a group has no string to
// resolve or scan for interpolation tokens.
type LeafKeys<T> = Extract<
  { [K in keyof T]: T[K] extends string ? K : never }[keyof T],
  keyof T
>;

const AGENT_KEYS = Object.keys(enAgents.tab_body.mcp_config).filter(
  (key) =>
    typeof enAgents.tab_body.mcp_config[
      key as keyof typeof enAgents.tab_body.mcp_config
    ] === "string",
) as Array<LeafKeys<typeof enAgents.tab_body.mcp_config>>;
const SETTINGS_KEYS = Object.keys(enSettings.mcp).filter(
  (key) =>
    typeof enSettings.mcp[key as keyof typeof enSettings.mcp] === "string",
) as Array<LeafKeys<typeof enSettings.mcp>>;
