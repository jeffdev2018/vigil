...
// The fork's IssuesPage stacks a run-halt banner and the getting-started card
// above the surface. Both are workspace-scoped queries of their own, covered
// where they live; this suite is about the page title's refresh state, so give
// the page its workspace identity and stub the two banners out.
vi.mock("@multica/core/hooks", () => ({ useWorkspaceId: () => "ws-1" }));
vi.mock("@multica/core/paths", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@multica/core/paths")>()),
  useCurrentWorkspace: () => ({ id: "ws-1", slug: "acme" }),
}));
vi.mock("../approvals", () => ({ RunHaltBanner: () => null }));
vi.mock("../onboarding", () => ({ GettingStartedCard: () => null }));
