...
export type OriginInfo = {
  type: "runtime_local" | "clawhub" | "skills_sh" | "github" | "distilled" | "skill_miner" | "manual";
  provider?: string;
  runtime_id?: string;
  source_path?: string;
  source_url?: string;
  ...
  skill?: string;
  slug?: string;
  // Provenance for skills distilled from a successful run (k69).
...
export function readOrigin(skill: SkillSummary): OriginInfo {
  ...
  if (raw?.type === "distilled") return raw;
  if (raw?.type === "skill_miner") return raw;
  return { type: "manual" };
}
