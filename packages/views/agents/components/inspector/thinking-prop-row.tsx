...
/**
 * Thinking control for agent create / inspector general form. Hidden when
 * the active model has no `supported_levels` advertised AND nothing is
 * persisted, so providers that don't expose reasoning never surface an
 * empty row. If the agent already has a `thinking_level` saved (model
 * swap into a non-thinking runtime, or the daemon / CLI catalog shrank
 * and dropped the entry), we still render so the user can see the orphan
 * token the backend is still sending and explicit-clear it via the
 * picker footer. PR1's per-model invalid behavior is daemon-side
 * warn/drop, not a synchronous DB clear, so the frontend has to surface
 * the persisted state honestly.
 *
