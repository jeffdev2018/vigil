...
describe("MemoryTab", () => {
  ...
  it("keeps a correction candidate draft on failure and preserves evidence when the run is gone", async () => {
    ...
    proposal.unmount();
    mockListAgentMemories.mockResolvedValue([{id:"memory-1",agent_id:"agent-1",content:"Preserve the project in new forms.",source:"manual",source_task_id:"run-1",status:"pending",revision:1,
      source_review:{review_id:"review-1",issue_id:"issue-1",task_id:"run-1",feedback:"The form lost the project.",criteria:["Keep context"],assessments:[{passed:false,evidence:"Project field was empty."}],reviewed_by:"Jeff",reviewed_at:"2026-09-05T00:00:00Z"}}]);
    renderMemoryTab();
  ...
  it("approves the displayed revision and lets a reader inspect without approving", async () => {
    ...
    mockListAgentMemories.mockResolvedValue([suggestion]);
    const user = userEvent.setup();
  ...
  it("previews a restoration, keeps it on error, and makes history readable without edit permission", async () => {
    ...
    mockListAgentMemories.mockResolvedValue([current]);
  ...
  it("keeps an expired lesson inactive when its text is edited without changing the date", async () => {
    mockListAgentMemories.mockResolvedValue([{ id: "mem-expired", agent_id: agent.id, content: "Old endpoint", source: "manual", source_task_id: null,
      created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z", status: "active", revision: 3, expires_at: "2026-01-02T00:00:00Z", expired: true }]);
