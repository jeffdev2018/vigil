...
export function PullRequestList({
  ...
  return (
    <div className="space-y-1">
      {expandedHead.map((pr) => (
        <PullRequestRow key={pr.id} pr={pr} issueId={issueId} wsId={wsId} autoFix={autoFix} actions={rowActions} />
      ))}
      {useCollapse ? (
        <div className="space-y-1">
          {expanded
            ? collapsedTail.map((pr) => <PullRequestRow key={pr.id} pr={pr} issueId={issueId} wsId={wsId} autoFix={autoFix} actions={rowActions} />)
            : null}
          <button
            type="button"
...
function PullRequestRow({
  ...
  issueId: string;
  wsId: string;
  autoFix: IssueCIAutoFix | undefined;
