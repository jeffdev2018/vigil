...
// EditTriggerDialog is the row's own edit dialog (MUL-7478): before it, an
// autopilot's routing rules were only reachable through AutopilotDialog, which
// speaks for `triggers[0]` alone. It edits both schedules and webhooks; this
// file covers its schedule discipline — the minimal-patch behavior ported from
// the now-removed EditScheduleTriggerDialog (see MUL-4302 below).
...
import { EditTriggerDialog } from "./trigger-row";
...
const saveButton = () => screen.getByRole("button", { name: "Save trigger" });
...
beforeEach(() => {
  ...
  mockRotateToken.mockReset().mockResolvedValue({ id: "trg-evening" });
  mockSetSigningSecret.mockReset().mockResolvedValue({ id: "trg-evening" });
...
describe("EditTriggerDialog (schedule)", () => {
...
describe("EditTriggerDialog sends only what the user changed", () => {
